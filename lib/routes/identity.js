...
routes.createIdentity = function(req, res, next) {
  ...
  identities.createIdentity(req.body, function(err, createdIdentity) {

    if (err) {
      return next(new errors.InternalError('Could not create requested Identity object'));
    }

    req.log.info('new identity registered', createdIdentity);
    res.send(201, {id: createdIdentity.id});
    return next();
  });
};
