...
module.exports = {
  ...
  findChannelsByFieldValue: findChannelsByFieldValue
};
...
function retrieveChannelDataForIdentity(identity, callback) {
  ...
  identities.get(identity, function(err, foundIdentity) {
    ...
    Channel.find().where('_id').in(foundIdentity.channels).exec(function(err, foundChannels) {
      ...
      if (err) {
        return callback(new errors.InternalError('Could not fetch the channels associated to the provided Identity object'));
      }
...
function updateChannel(channelId, changes, callback) {
  ...
  getChannel(channelId, function(err, foundChannel) {
    ...
    if (err) {
      return callback(new errors.InternalError('Could not fetch Channel object to edit'));
    }
    ...
    foundChannel.save(foundChannel, function(err, saveResult) {
      ...
      if (err) {
        return callback(new errors.InternalError('Could not update requested Channel object'));
      }
  ...
  async.waterfall([
    function findChannel(done) {
      getChannel(channelId, function(err, foundChannel) {
        ...
        if (err) {
          return done(new errors.InternalError('Could not fetch requested Channel object'));
        }
    ...
    function checkAssociations(channel, identity, done) {
      ...
      var identityHasChannel = _.some(identity.channels, function(channelItem) {
        return channelItem.toString() == channel.id;
      });
    ...
    function deleteIdentityFromChannel(channel, identity, done) {
      ...
      async.parallel({
        ...
        deleteChannelIdFromIdentity: function(microDone) {
          identities.removeValuesFromField(identityId, 'channels', channelId, microDone);
        }
      }, function(err) {

        if (err) {
          return done(new errors.InternalError('Deletion process failed'));
        }
      ...
      if (err) {
        return callback(new errors.InternalError('Could not delete provided Identity object from channel'));
      }
...
function findChannelsByFieldValue(field, values, callback) {

  Channel.find().where(field).in(values).exec(function(err, foundChannels) {

    if (err) {
      return callback(err);
    }

    return callback(null, foundChannels);
  });
}