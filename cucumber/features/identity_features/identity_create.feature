Feature: A requester asks for the creation of an identity object
  ...
  Scenario Outline: create and check a new identity object based on the request body contents
    ...
    And the response includes an Identity identifier

    Examples:
      | endpoint      | request_contents     | status |
      | /api/identity | valid_identity_email | 201    |
      | /api/identity | valid_identity_phone | 201    |
      | /api/identity | valid_identity_gcm   | 201    |
  ...
  Examples:
    | endpoint      | request_contents      | status |
    | /api/identity | valid_identity_sms    | 201    |
    | /api/identity | valid_identity_apn    | 201    |
    | /api/identity | duplicated_identity   | 500    |